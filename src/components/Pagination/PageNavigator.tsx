...
export const PagePrev = () => {
  ...
  return (
    <span className={`page-prev ${disabled ? "disabled" : ""}`} onClick={() => {
      if (disabled) return;
...
export const PageNext = () => {
  ...
  return (
    <span className={`page-next ${disabled ? 'disabled': ''}`} onClick={() => {
      if (disabled) {
        return;
      }
      setCurrent(current + 1)
...
export const PageDot = ({index}: {index: number}) => {
  ...
  return <span className="page-item" onClick={() => setCurrent(index)}>...</span>;
};
